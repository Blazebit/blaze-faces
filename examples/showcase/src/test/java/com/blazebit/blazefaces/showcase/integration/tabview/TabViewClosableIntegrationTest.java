...
public class TabViewClosableIntegrationTest extends AbstractIntegrationTest {
	...
	@Before
	public void init() {
		String testUrl = toShowcaseUrl("tabviewClosable.xhtml");
		driver.get(testUrl);
	}
	...
	public void shouldCloseTab() throws InterruptedException {
		...
		List<WebElement> tabContents = findElementByClass("ui-tabs-panels")
				.findElements(By.tagName("div"));
		tabs = tabView.findElements(By.tagName("li"));
		assertThat(tabs.size(), equalTo(2));
		assertThat(tabContents.size(), equalTo(2));

		assertThat(tabs.get(0).findElement(By.tagName("a")).getText(),
				equalTo("Godfather Part I"));
		assertThat(tabs.get(1).findElement(By.tagName("a")).getText(),
				equalTo("Godfather Part III"));
	}
}
