package com.blazebit.blazefaces.showcase.integration.dragdrop;
...
public class DragDropBasicIntegrationTest extends AbstractIntegrationTest {
	...
	public void shouldRegularDraggable() {
		...
		assertTrue("Regular draggable should move correct.", pe.x - ps.x == 50
				&& pe.y - ps.y == 50);
	}
	...
	public void shouldHeaderDraggable() {
		...
		assertTrue("Should not move without dragging header.", pe.x == ps.x
				&& pe.y == ps.y);
		...
		assertTrue("Should move with dragging header.", pe.x - ps.x == 50
				&& pe.y - ps.y == 50);
	}
	...
	public void shouldHorizontalDraggable() {
		...
		assertTrue("Should move only horizontal", pe.x - ps.x == 50
				&& pe.y == ps.y);
	}
	...
	public void shouldVerticalDraggable() {
		...
		assertTrue("Should move only vertical.", pe.x == ps.x
				&& pe.y - ps.y == 50);
	}
	...
	public void shouldCloneDraggable() throws InterruptedException {
		...
		assertTrue("Should clone right.", cd.height == od.height
				&& cd.width == od.width);

		action.releaseMouse();

		assertTrue("Should delete clone.",
				findElementsBySelector(".ui-draggable.ui-draggable-dragging")
						.isEmpty());
	...
	public void shouldRevertDraggable() throws InterruptedException {
		...
		assertTrue("Should labeled as dragging.",
				hasClass(revert, "ui-draggable-dragging"));
	...
	public void shouldOpacityDraggable() {
		...
		assertTrue("Should opacity lower.",
				Double.valueOf(opacity.getCssValue("opacity")) < 1);
		...
		assertTrue("Should move right.", (ps.x == pe.x - 10)
				&& (ps.y == pe.y - 10));
	}
	...
	public void shouldGridDraggable() throws InterruptedException {
		...
		assertTrue("Should not move if lower than grid.", pe.x == ps.x
				&& pe.y == ps.y);
	...
	public void shouldRestrictDraggable() throws InterruptedException {
		...
		assertTrue("Restrict draggable should move correct.", pe.x - ps.x == 50
				&& pe.y - ps.y == 50);
		...
		assertTrue("Restrict draggable should not move.", pe.x - ps.x < 150
				&& pe.y - ps.y < 150);
	...
	public void shouldImageDraggable() {
		...
		assertTrue("Image draggable should move correct.", pe.x - ps.x == 50
				&& pe.y - ps.y == 50);
