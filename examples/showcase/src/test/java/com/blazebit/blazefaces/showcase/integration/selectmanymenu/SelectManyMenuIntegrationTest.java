package com.blazebit.blazefaces.showcase.integration.selectmanymenu;
...
public class SelectManyMenuIntegrationTest extends AbstractIntegrationTest {
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			WebElement hidden_input = findElementBySelector(basic,
					"select[multiple]");
			List<WebElement> hidden_items = findElementsBySelector(
					hidden_input, "option:not([selected])");

			assertTrue("Should render valid number of items.",
					items.size() == 3);
			assertTrue("Should render valid number of hiddens.",
					hidden_items.size() == 3);
			...
			for (int i = 0; i < items.size(); i++) {
				...
				assertThat("Should labels match.", e.getText(),
						equalTo(h.getAttribute("value")));
			}
			...
			hidden_items = findElementsBySelector(scroll,
					"option:not([selected])");
	...
	public void shouldSelectMany() {
		...
		assertTrue("Results dialog be hidden on startup.",
				!dialog.isDisplayed());
		...
		assertTrue(
				...
				findElementsBySelector(dialog,
						escapeClientId("form:selectedOptions") + " li").size() == 1);

		assertTrue(
				"Should display valid results.",
				findElementsBySelector(dialog,
						escapeClientId("form:selectedPlayers") + " li").size() == 1);
	}

	protected int testGivenMenu(WebElement menu) {

		List<WebElement> items = findElementsBySelector(menu, "li");
		WebElement hidden_input = findElementBySelector(menu,
				"select[multiple]");
		List<WebElement> hidden_items = findElementsBySelector(hidden_input,
				"option");
		...
		for (int i = 0; i < items.size(); i++) {
			...
			assertTrue("Should not be active stated without selection.",
					!hasClass(e, "ui-state-active"));

			e.click();

			assertTrue("Should be selected.", h.isSelected());

			assertTrue("Should be active stated.",
					hasClass(e, "ui-state-active"));

			assertTrue(
					"Should be only one selected.",
					findElementsBySelector(menu, "li.ui-state-active").size() == 1);
		}
		 ...
		 * for (int i = 0; i < items.size(); i++) {
		 * 
		 * WebElement e = items.get(i); WebElement h = hidden_items.get(i);
		 * 
		 * if(i != items.size() - 1){
		 * assertTrue("Should not be active stated without selection.",
		 * !hasClass(e, "ui-state-active"));
		 ...
		 * assertTrue("Should be selected.", h.isSelected());
		 * 
		 * assertTrue("Should be active stated.", hasClass(e,
		 * "ui-state-active"));
		 * 
		 * assertTrue("Should be cumulative selected.",
		 * findElementsBySelector(menu, "li.ui-state-active").size() == i + 2);
		 * } else { assertTrue("Should be active stated at first.", hasClass(e,
		 * "ui-state-active"));
		 * 
		 * e.click();
		 * 
		 ...
		 * 
		 * assertTrue("Should be deactivated.", !hasClass(e,
		 * "ui-state-active"));
		 * 
		 * assertTrue("Should be cumulative selected.",
		 * findElementsBySelector(menu, "li.ui-state-active").size() ==
		 * items.size() - 1); } }
		 * 
