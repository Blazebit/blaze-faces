package com.blazebit.blazefaces.showcase.integration.selectmanycheckbox;
...
public class SelectManyCheckboxBasicIntegrationTest extends
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			List<WebElement> boxes = findElementsBySelector(horizontal,
					".ui-chkbox-box");
			List<WebElement> hiddens = findElementsBySelector(horizontal,
					"input[type='checkbox']:not([checked])");
			List<WebElement> labels = findElementsBySelector(horizontal,
					"label");

			assertTrue("Should render valid number of boxes.",
					boxes.size() == 3);
			assertTrue("Should render valid number of hiddens.",
					hiddens.size() == 3);
			assertTrue("Should render valid number of labels.",
					labels.size() == 3);
			...
			for (int i = 0; i < boxes.size(); i++) {
				...
				assertTrue("Icon should be hidden with false.",
						!icon.isDisplayed());

				assertTrue("Should not active stated.",
						!hasClass(b, "ui-state-active"));

				assertTrue("Input element should be hidden.", !h.isDisplayed());

				assertThat("Should labels match.", l.getText(),
			...
			hiddens = findElementsBySelector(vertical,
					"input[type='checkbox']:not([checked])");
			...
			assertTrue("Results dialog should be hidden.",
	...
	public void shouldManageMenu() {
		...
		hiddens.addAll(findElementsBySelector(vertical,
				"input[type='checkbox']:not([checked])"));
		...
		for (int i = 0; i < boxes.size(); i++) {
			...
			assertTrue("Icon should have checked.",
					hasClass(icon, "ui-icon-check"));
			...
			assertTrue("Icon should have unchecked.",
					!hasClass(icon, "ui-icon-check"));
		...
		assertTrue(
				...
				findElementsBySelector(
						dialog,
						escapeClientId("form:selectedOptions")
		...
		assertTrue(
				...
				findElementsBySelector(
						dialog,
						escapeClientId("form:selectedMovies")
								+ " .ui-datalist-item").size() == 4);
