...
public class AccordionTabChangeListenerIntegrationTest extends
		AbstractAccordionIntegrationTest {

	protected static final String TEST_URL = BLAZE_SHOWCASE_UI
			+ "accordionPanelChangeListener.xhtml";
	...
	public void shouldManageAccordion() {
		...
		for (int i = 1; i < headers.size(); i++) {
			...
			String growlTitle = findElementBySelector(growl.get(0),
					"span.ui-growl-title").getText();

			assertThat("Should execute listener correct with title.",
					growlTitle, equalTo("Tab Changed"));

			String growlMessage = findElementBySelector(growl.get(0), "p")
					.getText();

			assertThat("Should execute correct item.", growlMessage,
