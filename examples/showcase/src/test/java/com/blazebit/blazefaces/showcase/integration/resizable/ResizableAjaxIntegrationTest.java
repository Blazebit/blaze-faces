...
public class ResizableAjaxIntegrationTest extends AbstractIntegrationTest {
	...
	public static final String TEST_URL = BLAZE_SHOWCASE_UI + "resizeAjax.xhtml";
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			assertTrue("Resize handles should be placed.",
					findElementsBySelector(wrapper, ".ui-resizable-handle")
							.size() == 3);
			...
			assertTrue("Should image sourced.", !image.getAttribute("src")
					.isEmpty());

			assertTrue("Should render hidden growl.", !findElementById("growl")
					.isDisplayed());
	...
	public void shouldAjaxResize() {
		...
		assertTrue("Should ajax values rendered.",
				findElementsBySelector(".ui-growl-message").size() == 1);

		assertTrue("Should ajax values displayed.",
				findElementBySelector(".ui-growl-message").isDisplayed());
