...
public class TreeTableEventsIntegrationTest extends AbstractIntegrationTest {
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			assertTrue("Growl should be hidden on startup.",
					!findElementById("form:messages").isDisplayed());
	...
	protected void shouldManageTree(WebElement e) {
		...
		try {
			...
			assertTrue("Should growl contains right results.", growl.getText()
					.equals("Selected\n" + nodeName));
		...
		if (toggler.isDisplayed()) {
			...
			if (Boolean.valueOf(expanded)) {
				...
				try {
					...
					assertTrue("Should growl contains right results.", growl
							.getText().equals("Expanded\n" + nodeName));
			...
			findElementBySelector(e, "td:first-child .ui-treetable-toggler")
					.click();
			waitUntilAjaxRequestCompletes();
			waitUntilAllAnimationsComplete();
