...
public class EffectsCatalogIntegrationTest extends AbstractIntegrationTest {
	...
	protected static final String TEST_URL = BLAZE_SHOWCASE_UI + "effects.xhtml";
	...
	public void shouldEffectsWork() throws InterruptedException {
		...
		assertTrue("Should animate correct.",
				shouldElementAnimating(drop, "left", DECREASING, 300));
		...
		assertTrue("Should backward to initial left value.",
				slide.getCssValue("left").equalsIgnoreCase(slideLeft));
		...
		assertTrue("Should backward to initial top value.",
				bounce.getCssValue("top").equalsIgnoreCase(top));
		...
		assertTrue("Should animate correct.",
				shouldElementAnimating(pulsate, "opacity", DECREASING, 200));
		...
		assertTrue("Should animate correct.",
				shouldElementAnimating(shake, "left", DECREASING, 300));
		waitUntilAnimationCompletes("#shake");
		assertTrue("Should backward to initial left value.",
				shake.getCssValue("left").equalsIgnoreCase(left));
		...
		assertTrue("Should animate correct.",
				shouldElementAnimating(size, "width", INCREASING, 200));
		assertTrue("Should animate correct.",
				shouldElementAnimating(size, "height", INCREASING, 200));
