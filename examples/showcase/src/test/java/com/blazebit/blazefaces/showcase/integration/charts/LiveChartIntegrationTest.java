...
public class LiveChartIntegrationTest extends AbstractIntegrationTest {
	...
	@Before
	public void init() {
		driver.get(toShowcaseUrl("liveChart.xhtml"));
		actionHelper = new SeleniumActionHelper(driver);
	}

	@Test
	public void shoudChangeChartValues() {
		findElementById("form:votes").findElement(
				By.className("jqplot-pieRenderer-highlight-canvas"));

		WebElement chart = findElementById("form:votes").findElement(
		...
		WebElement clickedPlace = findElementById("form:votes").findElement(
				By.className("ui-chart-tooltip"));
		String firstValue = clickedPlace.getText();

		waitForSeconds(3);

		chart = findElementById("form:votes").findElement(
				By.className("jqplot-event-canvas"));
		actionHelper.mouseHover(chart);
		actionHelper.clickOnCurrentPosition();
		waitForCondition(new ExpectedCondition<Boolean>() {
		...
		WebElement secondClickedPlace = findElementById("form:votes")
				.findElement(By.className("ui-chart-tooltip"));
