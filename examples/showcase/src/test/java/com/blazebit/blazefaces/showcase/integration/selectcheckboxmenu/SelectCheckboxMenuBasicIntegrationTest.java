package com.blazebit.blazefaces.showcase.integration.selectcheckboxmenu;
...
public class SelectCheckboxMenuBasicIntegrationTest extends
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			List<WebElement> items = findElementsBySelector(menu,
					"input[name='form:menu'][type='checkbox']");

			assertTrue("Should render hidden checkbox inputs.",
					items.size() == 4);
			for (WebElement e : items) {
				assertTrue("Checkbox input elements should be hidden.",
						!e.isDisplayed());
			}

			WebElement menupanel = findElementById("form:menu_panel");

			assertTrue("Menu panel should be hidden on startup.",
					!menupanel.isDisplayed());

			assertTrue(
					"Should render correct size of checkbox boxes.",
					findElementsBySelector(menupanel, ".ui-chkbox-box").size() == items
							.size());
	...
	public void shouldManageMenuPanel() {
		...
		List<WebElement> items = findElementsBySelector(panel,
				".ui-selectcheckboxmenu-item");
		List<WebElement> hiddens = findElementsBySelector(menu,
				"input[type='checkbox']");
		...
		for (int i = 0; i < items.size(); i++) {
			...
			assertTrue("Should not be actived on false.",
					!hasClass(box, "ui-state-active"));
			assertTrue("Icon should be hidden on false.", !icon.isDisplayed());

			box.click();

			assertTrue("Should active checkbox by check.",
					hasClass(box, "ui-state-active"));
			...
			assertThat("Labels should match.", label.getText(),
					equalTo(h.getAttribute("value")));

			label.click();

			assertTrue("Should remove active class on checkbox.",
					!hasClass(box, "ui-state-active"));
			assertTrue("Icon should be hidden on false.", !icon.isDisplayed());
		...
		assertTrue("Menu panel should be hidden by clicking outside.",
				!panel.isDisplayed());
		...
		assertTrue(
				...
				findElementsBySelector(dialog, " .ui-datalist-item").size() == 4);
	}
}
