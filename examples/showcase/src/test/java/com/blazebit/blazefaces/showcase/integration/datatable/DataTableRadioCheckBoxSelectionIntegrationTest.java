...
public class DataTableRadioCheckBoxSelectionIntegrationTest extends
		AbstractDataTableIntegrationTest {

	@Before
	public void before() {
		driver.get(toShowcaseUrl("datatableRowSelectionRadioCheckbox.xhtml"));
	}
	...
	public void shouldDisplayRadioButtonSelectedRows() {
		...
		List<String> aRow = getRowByRowIndex(rows, rowIndex, FIRST_COLUMN + 1,
				END_COLUMN + 1);
	...
	public void shouldDisplayCheckBoxSelectedRows() {
		List<WebElement> rows = findElementById("form:multiCars_data")
				.findElements(By.cssSelector(ROW_CLASS));
		Integer rowIndex = new Random().nextInt(rows.size());
		String model1 = getItemByColumnAndRow(rows, rowIndex,
				Columns.MODEL.ordinal() + 1);
		clickWithScroll(rows.get(rowIndex).findElement(
				By.xpath("td/div/div/div")));

		rowIndex = getAnotherRandomNumber(rowIndex, rows.size());
		String model2 = getItemByColumnAndRow(rows, rowIndex,
				Columns.MODEL.ordinal() + 1);
		clickWithScroll(rows.get(rowIndex).findElement(
				By.xpath("td/div/div/div")));
