package com.blazebit.blazefaces.showcase.integration.overlaypanel;
...
public class OverlayPanelBasicIntegrationTest extends AbstractIntegrationTest {
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			assertTrue("Car panel should be hidden on startup.",
					!findElementById("form:carPanel").isDisplayed());

			findElementById("form:chartBtn");
			assertTrue("Chart panel should be hidden on startup.",
					!findElementById("form:chartPanel").isDisplayed());

			findElementById("form:img");
			assertTrue("Image panel should be hidden on startup.",
					!findElementById("form:imgPanel").isDisplayed());
	...
	public void shouldDisplayPanels() throws InterruptedException {
		...
		assertTrue("Should only display chart panel.", chart.isDisplayed()
				&& !car.isDisplayed() && !image.isDisplayed());
		...
		waitUntilAllAnimationsComplete();
		assertTrue("Should only display car panel.", car.isDisplayed()
				&& !chart.isDisplayed() && !image.isDisplayed());
		...
		waitUntilAllAnimationsComplete();
		assertTrue("Should only display image panel.", image.isDisplayed()
				&& !car.isDisplayed() && !chart.isDisplayed());

		title.click();
		waitUntilAllAnimationsComplete();
		assertTrue(
				"Should none display.",
				!image.isDisplayed() && !car.isDisplayed()
						&& !chart.isDisplayed());
