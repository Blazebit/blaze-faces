...
public class TreeTableBasicIntegrationTest extends AbstractIntegrationTest {
	...
	public static final String TEST_URL = BLAZE_SHOWCASE_UI + "treeTable.xhtml";
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			assertTrue("TreeTable should be displayed on startup.",
					treetable.isDisplayed());

			// table should be labeled.
			assertTrue(
					"TreeTable should be all labeled/roled.",
					findElementsBySelector(
							treetable,
							"table[role='treegrid'] tbody tr[role='row'][aria-expanded='false'] td[role='gridcell']")
							.size() == 12);

			assertTrue("Dialog should be hidden on startup.",
	...
	public void shouldManageTreeTable() {
		for (WebElement e : findElementsBySelector(
				findElementById("form:treetable"), "tbody > tr")) {
			shouldManageTree(e);
		}
	}
	...
	protected void shouldManageTree(WebElement e) {
		...
		assertThat(
				"Dialog should be updated.",
				nodeName,
				equalTo(findElementBySelector(dialog,
						"table tr:first-child td:first-child + td").getText()));

		findElementBySelector(dialog, "a.ui-dialog-titlebar-close").click();

		waitUntilAllAnimationsComplete();

		WebElement toggler = findElementBySelector(e,
				"td:first-child .ui-treetable-toggler");
		...
		if (toggler.isDisplayed()) {
			...
			List<WebElement> children = findElementsByXpath(
					findElementById("form:treetable"),
					"//tbody/tr[starts-with(@id, '" + id + "')]");
			...
			children = findElementsByXpath(findElementById("form:treetable"),
					"//tbody/tr[starts-with(@id, '" + id + "')]");
