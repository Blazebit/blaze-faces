...
public class DataTableFilteringIntegrationTest extends
	...
	public void shouldContains() {
		...
		int validCount = findElementsByXpath(table,
				"table/tbody//tr/td[1][contains(., 'a')]").size();

		findElementById("form:dataTable:modelColumn_filter").sendKeys("a");

		waitUntilAjaxRequestCompletes();

		List<WebElement> cols = findElementsByXpath(table,
				"table/tbody//tr/td[1][contains(., 'a')]");

		assertTrue("Should find valid amount.", cols.size() == validCount);

		for (WebElement e : cols) {
			assertTrue("Should find valid (contains) items.", e.getText()
					.contains("a"));
		}
	}
	...
	public void shouldStartsWith() {
		...
		int validCount = findElementsByXpath(table,
				"table/tbody//tr/td[2][starts-with(., '199')]").size();
		...
		List<WebElement> cols = findElementsByXpath(table,
				"table/tbody//tr/td[2][starts-with(., '199')]");

		assertTrue("Should find valid amount.", cols.size() == validCount);

		for (WebElement e : cols) {
			assertTrue("Should find valid (starts with) items.", e.getText()
					.startsWith("199"));
		}
	}
	...
	public void shouldExact() {
		...
		List<WebElement> cols = findElementsByXpath(table,
				"table/tbody//tr/td[3][. = 'Ford']");

		assertTrue("Should find valid amount.", cols.size() == validCount);

		for (WebElement e : cols) {
			assertTrue("Should find valid (exact) items.",
					e.getText().equals("Ford"));
		}
	}
	...
	public void shouldEndsWith() {
		...
		int validCount = findElementsByXpath(table,
				"table/tbody//tr/td[4][. = 'Yellow']").size();
		...
		List<WebElement> cols = findElementsByXpath(table,
				"table/tbody//tr/td[4][. = 'Yellow']");

		assertTrue("Should find valid amount.", cols.size() == validCount);

		for (WebElement e : cols) {
			assertTrue("Should find valid (ends with) items.", e.getText()
					.endsWith("low"));
		}
	}
}
