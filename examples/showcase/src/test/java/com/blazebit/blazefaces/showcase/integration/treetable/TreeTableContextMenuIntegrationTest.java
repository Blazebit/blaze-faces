...
public class TreeTableContextMenuIntegrationTest extends
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			assertTrue("Should context menu not displayed on startup.",
					!findElementById("form:contextMenuId").isDisplayed());

		} catch (NoSuchElementException e) {
			assertTrue("TreeTable ContextMenu showcase DOM not verified.",
					false);
		}
	}
	...
	public void shouldManageTreeTable() throws InterruptedException {
		List<WebElement> root = findElementsBySelector(
				findElementById("form:docs"), "tbody > tr");
		for (int i = root.size(); i > 0; i--) {
			shouldManageTree(findElementsBySelector(
					findElementById("form:docs"), "tbody > tr").get(i - 1));
		}
	}
	...
	protected void shouldManageTree(WebElement e) throws InterruptedException {
		...
		if (toggler.isDisplayed()) {
			...
			children = findElementsByXpath(findElementById("form:docs"),
					"//tbody/tr[starts-with(@id, '" + id + "')]");
