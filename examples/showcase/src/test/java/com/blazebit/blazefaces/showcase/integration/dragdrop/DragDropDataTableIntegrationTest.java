...
public class DragDropDataTableIntegrationTest extends AbstractIntegrationTest {
	...
	protected static final String TEST_URL = BLAZE_SHOWCASE_UI + "dndTable.xhtml";
	...
	public void shouldVerifyDOM() {
		...
		try {
			...
			source = findElementBySelector(form,
			...
			target = findElementBySelector(form,
					escapeClientId("carForm:selectedCars"));

			dropArea = findElementBySelector(target,
					escapeClientId("carForm:dropArea"));
	...
	public void shouldDragItemValid() throws InterruptedException {
		...
		for (int i = items.size(); i > 0; i--) {
			...
			WebElement e = findElementBySelector(source,
					escapeClientId("carForm:availableCars_data")
							+ " > tr:nth-last-child(n)");

			WebElement drag = findElementBySelector(e,
					"td:first-child .ui-draggable");

			String text = findElementBySelector(e, "td:first-child + td")
					.getText();
			...
			try {
				findElementByXpath(source,
						"/table//div[contains(@class, 'ui-dt-c')][contains(.,'"
								+ text + "')]");
			...
			try {
				findElementByXpath(dropArea,
						"//div[contains(@class, 'ui-dt-c')][contains(.,'"
								+ text + "')]");
			} catch (NoSuchElementException ex) {
