/**
 * BlazeFaces Slider Widget
 */
BlazeFaces.widget.Slider = BlazeFaces.widget.BaseWidget.extend({
    
    init: function(cfg) {
        this._super(cfg);
        
        this.input = $(BlazeFaces.escapeClientId(this.cfg.input));
        if(this.cfg.output) {
            this.output = $(BlazeFaces.escapeClientId(this.cfg.output));
        }
        var _self = this;
