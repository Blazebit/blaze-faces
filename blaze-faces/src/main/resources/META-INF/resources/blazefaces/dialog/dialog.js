            /**
 * BlazeFaces Dialog Widget
 */ 
BlazeFaces.widget.Dialog = BlazeFaces.widget.BaseWidget.extend({
    ...
    init: function(cfg) {
        ...
        if(this.cfg.width == 'auto' && BlazeFaces.isIE(7)) {
    ...
    bindEvents: function() {   
        ...
        //Move dialog to top if target is not a trigger for a BlazeFaces overlay
        this.jq.mousedown(function(e) {
            if(!$(e.target).data('blazefaces-overlay-target')) {
                _self.moveToTop();
            }
        });
    ...
    moveToTop: function() {
        this.jq.css('z-index', ++BlazeFaces.zindex);
    },
...
/**
 * BlazeFaces ConfirmDialog Widget
 */
BlazeFaces.widget.ConfirmDialog = BlazeFaces.widget.Dialog.extend({
