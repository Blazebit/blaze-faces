/**
 * BlazeFaces OverlayPanel Widget
 */
BlazeFaces.widget.OverlayPanel = BlazeFaces.widget.BaseWidget.extend({
    
    init: function(cfg) {
        this._super(cfg);
        
        this.targetId = BlazeFaces.escapeClientId(this.cfg.target);
    ...
    bindEvents: function() {
        //mark target and descandants of target as a trigger for a blazefaces overlay
        this.target.data('blazefaces-overlay-target', this.id).find('*').data('blazefaces-overlay-target', this.id);
    ...
    align: function() {
        ...
        this.jq.css({'left':'', 'top':'', 'z-index': ++BlazeFaces.zindex})
                .position({
                    my: this.cfg.my
                    ,at: this.cfg.at
