/**
 * BlazeFaces Menu Widget
 */
BlazeFaces.widget.Menu = BlazeFaces.widget.BaseWidget.extend({
    ...
    initOverlay: function() {
        ...
        this.trigger = $(BlazeFaces.escapeClientId(this.cfg.trigger));

        //mark trigger and descandants of trigger as a trigger for a blazefaces overlay
        this.trigger.data('blazefaces-overlay-target', true).find('*').data('blazefaces-overlay-target', true);
    ...
    show: function() {
        this.align();
        this.jq.css('z-index', ++BlazeFaces.zindex).show();
    },
...
/**
 * BlazeFaces TieredMenu Widget
 */
BlazeFaces.widget.TieredMenu = BlazeFaces.widget.Menu.extend({
    ...
    bindItemEvents: function() {
        ...
        if(this.cfg.autoDisplay == false) {
            ...
            this.rootLinks.data('blazefaces-menubar', this.id).find('*').data('blazefaces-menubar', this.id)
    ...
    bindDocumentHandler: function() {
        ...
        $(document.body).click(function(e) {
            var target = $(e.target);
            if(target.data('blazefaces-menubar') == _self.id) {
                return;
            }
...
/**
 * BlazeFaces Menubar Widget
 */
BlazeFaces.widget.Menubar = BlazeFaces.widget.TieredMenu.extend({
    
    showSubmenu: function(menuitem, submenu) {
        submenu.css('z-index', ++BlazeFaces.zindex);
...
/**
 * BlazeFaces SlideMenu Widget
 */
BlazeFaces.widget.SlideMenu = BlazeFaces.widget.Menu.extend({
...
/**
 * BlazeFaces PlainMenu Widget
 */
BlazeFaces.widget.PlainMenu = BlazeFaces.widget.Menu.extend({
...
/*
 * BlazeFaces MenuButton Widget
 */
BlazeFaces.widget.MenuButton = BlazeFaces.widget.BaseWidget.extend({
...
/*
 * BlazeFaces ContextMenu Widget
 */
BlazeFaces.widget.ContextMenu = BlazeFaces.widget.TieredMenu.extend({
    ...
    init: function(cfg) {
        ...
        //target
        this.jqTargetId = documentTarget ? document : BlazeFaces.escapeClientId(this.cfg.target);
    ...
    bindDataTable: function() {
        var dataTable = this.jqTarget.data('blazefaces-widget'),
    ...
    bindTreeTable: function() {
        var treeTable = this.jqTarget.data('blazefaces-widget'),
    ...
    bindTree: function() {
        var tree = this.jqTarget.data('blazefaces-widget'),
    ...
    refresh: function(cfg) {
        var jqId = BlazeFaces.escapeClientId(cfg.id),
        instances = $(jqId);
...
/**
 * BlazeFaces MegaMenu Widget
 */
BlazeFaces.widget.MegaMenu = BlazeFaces.widget.BaseWidget.extend({
    ...
    bindEvents: function() {
        ...
        if(this.cfg.autoDisplay == false) {
            this.rootLinks.data('blazefaces-megamenu', this.id).find('*').data('blazefaces-megamenu', this.id)
        ...
        $(document.body).click(function(e) {
            var target = $(e.target);
            if(target.data('blazefaces-megamenu') == _self.id) {
                return;
            }
...
/**
 * BlazeFaces PanelMenu Widget
 */
BlazeFaces.widget.PanelMenu = BlazeFaces.widget.BaseWidget.extend({
    ...
    saveState: function() {
        ...
        BlazeFaces.setCookie(this.stateKey, expandedNodeIds);
    },
    
    restoreState: function() {
        var expandedNodeIds = BlazeFaces.getCookie(this.stateKey);
        
        if(expandedNodeIds) {
            this.expandedNodes = expandedNodeIds.split(',');
            for(var i = 0 ; i < this.expandedNodes.length; i++) {
                var element = $(BlazeFaces.escapeClientId(this.expandedNodes[i]));
    ...
    clearState: function() {
        BlazeFaces.setCookie(this.stateKey, null);
    }

});

/**
 * BlazeFaces TabMenu Widget
 */
BlazeFaces.widget.TabMenu = BlazeFaces.widget.Menu.extend({
