/**
 * BlazeFaces Google Maps Widget
 */
BlazeFaces.widget.GMap = BlazeFaces.widget.BaseWidget.extend({
    ...
    loadWindow: function(content){
        this.jq.find(BlazeFaces.escapeClientId(this.getInfoWindow().id + '_content')).html(content||'');
    },
    ...
    configureEventListeners: function() {
        ...
        this.cfg.formId = $(BlazeFaces.escapeClientId(this.id)).parents('form:first').attr('id');
