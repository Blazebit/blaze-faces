/**
 * BlazeFaces AutoComplete Widget
 */
BlazeFaces.widget.AutoComplete = BlazeFaces.widget.BaseWidget.extend({
    ...
    init: function(cfg) {
        ...
        this.hinput.data(BlazeFaces.CLIENT_ID_DATA, this.id);
        
        if(!this.disabled) {
            if(this.cfg.multiple) {
                this.setupMultipleMode();

                this.multiItemContainer.data('blazefaces-overlay-target', true).find('*').data('blazefaces-overlay-target', true);
            } 
            else {
                //visuals
                BlazeFaces.skinInput(this.input);

                this.input.data('blazefaces-overlay-target', true).find('*').data('blazefaces-overlay-target', true);
                this.dropdown.data('blazefaces-overlay-target', true).find('*').data('blazefaces-overlay-target', true);
            }
                        ...
                        if(_self.items.length > 0) {
                            ...
                            if(_self.panel.children().is('ul')) {
                                _self.items.each(function() {
                                    var item = $(this),
                                    text = item.html(),
                                    re = new RegExp(BlazeFaces.escapeRegExp(query), 'gi'),
